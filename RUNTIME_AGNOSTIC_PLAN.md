...
npm artifact with mainstream Node tooling, and publish the *same source* to JSR
...
publish the *same source* to JSR via `npx jsr publish`. This is the inverse of a
...
| Task runner | **moon** (moonrepo). Models build/test/publish as a cached DAG. Polyglot-ready for the planned Rust port. |
| Release tooling | **Release Please.** Conventional-commits driven, language-agnostic. The `node-workspace` plugin propagates dependent bumps and rewrites intra-repo version refs; `cargo-workspace` + `linked-versions` extend the *same* release PR to the planned Rust crate. |
...
│   ├── config/
│   ├── client/
│   └── worker/         # each: package.json + jsr.json + tsconfig + moon.yml
├── apps/docs/
├── .moon/              # root tasks: test, build, publish-npm, publish-jsr
...
7. **Adopt moon.**
   Root tasks: `test`, `build`, `publish-npm`, `publish-jsr`. Per-package
   `moon.yml` inherits and overrides, and declares inter-package build edges
   (e.g. `worker:build` depends on `core:build`) so the DAG builds siblings in
   the right order.
8. **Wire publishing in CI.**
   On a merged Release Please release PR, moon runs `publish-npm` and
   `publish-jsr`. `publish-npm` runs **`pnpm publish` per-package** with
   provenance via GitHub OIDC (`id-token: write` + `--provenance`); running
   through pnpm (not bare `npm publish`) is what rewrites the `workspace:*`
   specifiers to real version ranges in the published tarball. `publish-jsr`
   first generates `jsr.json` from `package.json`, then runs `npx jsr publish`
   from the same source (also GitHub OIDC).
9. **Documentation.**
...
- No API redesign. Surface stays as-is apart from the `internal` → `core`
  rename.
- No Rust port in this milestone — only the tooling that *unblocks* it later.
- No changes to the Redis protocol, Lua scripts, or queue semantics.
- No performance/benchmark suites in this milestone. *Future reference:* a
...
- **`workspace:*` not rewritten** to a real range on publish would ship broken
  ...
  concrete range in the tarball** (`npm publish` does *not*). Mitigation: publish
